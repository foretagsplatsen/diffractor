define([
	...
	function assertMatch(url, route, message) {
		expect(diffractor.url({rawUrl: url}).matchRoute(diffractor.route({ pattern: route })).isMatch()).toBeTruthy();
	}
	...
	describe("route", function() {
		...
		it("Route match result", function() {
			var route = diffractor.route({ pattern: "#a/#b" });
			var url = diffractor.url({rawUrl: "hello/world"});
		...
		it("Route match capture parameters", function() {
			var result = diffractor.url({rawUrl: "/hello/world"}).matchRoute(diffractor.route({pattern: "#foo/#bar" }));
		...
		it("Route match capture parameters mixed with statics", function() {
			var result = diffractor.url({rawUrl: "/hello/static/world"}).matchRoute(diffractor.route({ pattern: "#foo/static/#bar" }));
			var props = result.getRouteParameters();
		...
		it("Route parameter capture optional parameters mixed with parameters", function() {
			var firstOptionalBothMatch = diffractor.url({rawUrl: "hello/world"}).matchRoute(diffractor.route({ pattern: "?foo/#bar"})).getRouteParameters();
			expect(firstOptionalBothMatch).toEqual({ foo: "hello", bar : "world"});

			var firstOptionalOneMatch = diffractor.url({rawUrl: "/world"}).matchRoute(diffractor.route({ pattern: "?foo/#bar"})).getRouteParameters();
			expect(firstOptionalOneMatch).toEqual({ foo: undefined, bar : "world"});

			var optionalInPath = diffractor.url({rawUrl: "hello/world"}).matchRoute(diffractor.route({ pattern: "#foo/?bar/#bro" })).getRouteParameters();
			expect(optionalInPath).toEqual({ foo: "hello", bar: undefined,bro : "world"});

			var trailingOptionals = diffractor.url({rawUrl: "hello/world"}).matchRoute(diffractor.route({ pattern: "#foo/?bar/?bro" })).getRouteParameters();
			expect(trailingOptionals).toEqual({ foo: "hello", bar : "world", bro: undefined});
		});

		it("Route parameter can have defaults", function() {
			var route = diffractor.route({
				pattern: "?foo/?bar",
				options: {
					defaults: {
						bar: "world"
			...
			var result = diffractor.url({rawUrl: "/hello"}).matchRoute(route);
			var props = result.getRouteParameters();
		...
		it("Query", function() {
			var query = diffractor.url({rawUrl: "hello/world?a=1&b=2&c=3"}).getQuery();
		...
		it("Expand parameters", function() {
			var route = diffractor.route({ pattern: "#a/test/#b"});
		...
		it("Expand optionals", function() {
			var route = diffractor.route({ pattern: "#a/?c/#b/?d"});
		...
		it("Route with function constraint", function() {
			...
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "/hello/world"})).isMatch()).toBeTruthy();
			expect(!aRoute.matchUrl(diffractor.url({rawUrl: "/hello/sweden"})).isMatch()).toBeTruthy();
		});
		...
		it("Route with array constraint", function() {
			...
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "/hello/world"})).isMatch()).toBeTruthy();
			expect(!aRoute.matchUrl(diffractor.url({rawUrl: "/hello/france"})).isMatch()).toBeTruthy();
		});
		...
		it("Route with RegExp constraint", function() {
			...
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "/hello/world"})).isMatch()).toBeTruthy();
			expect(!aRoute.matchUrl(diffractor.url({rawUrl: "/hello/öland"})).isMatch()).toBeTruthy();
		});
		...
		it("Route with mixed constraints", function() {
			...
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "/henrik/mikael/h"})).isMatch()).toBeTruthy();
			expect(!aRoute.matchUrl(diffractor.url({rawUrl: "/ben/mikael/1"})).isMatch()).toBeTruthy();
			expect(!aRoute.matchUrl(diffractor.url({rawUrl: "/henrik/dennis/1"})).isMatch()).toBeTruthy();
			expect(!aRoute.matchUrl(diffractor.url({rawUrl: "/henrik/mikael/a"})).isMatch()).toBeTruthy();
		});
		...
		it("Route constraints on optional parameters", function() {
			...
			expect(aRoute.matchUrl(diffractor.url({rawUrl: ""})).isMatch()).toBeTruthy();

			expect(!aRoute.matchUrl(diffractor.url({rawUrl: "ö"})).isMatch()).toBeTruthy();

			expect(aRoute.matchUrl(diffractor.url({rawUrl: "henrik/micke/h"})).isMatch()).toBeTruthy();
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "henrik/micke/h"})).getRouteParameters()).toEqual({ a: "henrik", b: "micke", c: "h"});

			expect(aRoute.matchUrl(diffractor.url({rawUrl: "henrik/micke"})).isMatch()).toBeTruthy();
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "henrik/micke"})).getRouteParameters()).toEqual({ a: "henrik", b: "micke", c: undefined});

			expect(aRoute.matchUrl(diffractor.url({rawUrl: "henrik"})).isMatch()).toBeTruthy();
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "henrik"})).getRouteParameters()).toEqual({ a: "henrik", b: undefined, c: undefined});

			expect(aRoute.matchUrl(diffractor.url({rawUrl: "micke"})).isMatch()).toBeTruthy();
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "micke"})).getRouteParameters()).toEqual({ a: undefined, b: "micke", c: undefined});

			expect(aRoute.matchUrl(diffractor.url({rawUrl: "h"})).isMatch()).toBeTruthy();
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "h"})).getRouteParameters()).toEqual({ a: undefined, b: undefined, c: "h"});
		});
		...
		it("Ignore trailing segments route option", function() {
			...
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "/hello/world"})).isMatch()).toBeTruthy();
			expect(aRoute.matchUrl(diffractor.url({rawUrl: "/hello/world/and/some/extra"})).isMatch()).toBeTruthy();
		});
	});
});
